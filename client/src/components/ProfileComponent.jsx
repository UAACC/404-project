...
class ProfileCard extends React.Component {
  ...
  handleUpdateProfile = async () => {
    ...
    const doc = await axios.post(id + "/", { email, displayName, github, password }, config);
    if (doc.data) {
  ...
  handleAddFriend = async () => {
    ...
    if (domain === "social-distribution-t1.herokuapp.com"){
      ...
      doc = await axios.post("https://social-distribution-t1.herokuapp.com/author/"+id_digits+"/inbox/",{
        type: "follow",
        actor: currentUser,
        object: id,
      ...
      await axios.put("https://social-distribution-t1.herokuapp.com/author/" + id_digits + "/followers/" + currentUser?.id.split("/")[4] + "/", {remote: true}, config);
      
    } else {
      doc = await axios.post("https://" + domain + "/friendrequest/", {
        actor: currentUser.id,
        object: id
      }, config);
    }
  ...
  handleDeleteFriend = async () => {
    ...
    if (id.includes("t1")) {
      console.log(auth);
      const t1_url = "https://social-distribution-t1.herokuapp.com/author/" + id.split("/")[4] + "/friends/" + currentUser?.id.split("/")[4] + "/";
